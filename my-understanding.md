My Understanding
Submission Links
Loom Video [(https://www.loom.com/share/f60ad47eea2f4235923d402db0137032)] , [(https://www.loom.com/share/6449ffdeb23d44d095a7a1363125a077)]
...
ตัวอย่าง:
POST /products
body: { "name": "Keyboard", "price": 49.88, "quantity": 1 }
...
โดย Keyboard จะอยู่ใน req.query.name และ API ของผมใช้ค่านี้ไปค้นหาสินค้าตามชื่อ
...
Your answer: เมื่อส่ง POST ไปที่ /products server จะรับ request เข้ามาก่อน จากนั้น request จะผ่าน middleware เช่น cors(), express.json() และ middleware ที่ใช้ console.log method กับ path หลังจากนั้น Express จะส่ง request ไปที่ router ของ /products แล้วเข้า route router.post('/') ในไฟล์ product.js ใน route นี้ backend จะดึง name, price และ quantity ออกมาจาก req.body ก่อน ถ้าข้อมูลไม่ครบจะส่ง status 400 กลับไป แต่ถ้าครบก็จะใช้ Product.create() เพื่อสร้างสินค้าใหม่ใน MongoDB ถ้าสร้างสำเร็จจะตอบกลับด้วย status 201 และ message ว่า "Succesfully" แต่ถ้าเกิด error ใน server จะตอบกลับ status 500

8. What is CRUD? Map each operation to the HTTP method and route you used in your API.

Your answer: CRUD คือการจัดการข้อมูล 4 แบบ ได้แก่ Create, Read, Update และ Delete ซึ่ง API ของผม map กับ method และ route แบบนี้ครับ

Create ใช้ POST /products เพื่อสร้างสินค้าใหม่
Read ใช้ GET /products เพื่อดึงสินค้าทั้งหมด และ GET /products/:id เพื่อดึงสินค้าตาม id
Update ใช้ PUT /products/:id หรือ PATCH /products/:id เพื่อแก้ไขข้อมูลสินค้าตาม id
Delete ใช้ DELETE /products/:id เพื่อลบสินค้าตาม id

9. How does your API respond when something goes wrong — for example, when a product with a given ID does not exist?

Your answer: ถ้าไม่พบ product ตาม ID ที่ส่งมา API จะตอบกลับด้วย status code 404 และส่ง message เช่น "ไม่พบสินค้า" เพื่อแจ้งให้ frontend รู้ว่าไม่มีข้อมูลนี้อยู่ใน database ส่วนถ้าเป็นกรณีที่ส่งข้อมูลตอนสร้างสินค้าไม่ครบ เช่นไม่มี name, price หรือ quantity API จะตอบกลับ 400 พร้อม message ให้กรอกข้อมูลให้ครบ และถ้าเป็น error ฝั่ง server จะตอบกลับ 500
